...
import { BayaaCorner } from './components/BayaaCorner';
...
export default function App() {
  ...
  return <MotionConfig reducedMotion="user"><div className="app-shell">
    ...
    <div className="workspace"><header className="topbar"><div><span className="breadcrumb">Миний файлууд</span><span className="breadcrumb-slash">/</span><span>Таталтууд</span></div><div className="header-tools"><WelcomeGuide/><button id="tray-button" className="header-tool" disabled={!desktop} onClick={()=>{if(!api().MinimiseToTray){setError('Tray дэмждэг шинэ EXE-г нээнэ үү.');return;}api().MinimiseToTray().catch(err=>setError(String(err)));}}><ArrowDownToLine size={15}/> Tray-д хураах</button><div className="ready-indicator"><span className="live-dot"/>{desktop?'Холбогдсон':'Загварын харагдац'}</div></div></header><main className="main-content"><div className="page-heading"><div><p className="eyebrow">ХУРДТАЙ. ЦЭГЦТЭЙ. ТАНЫ ХЯНАЛТАД.</p><h1>{title}</h1><p>Таталтаа нэг дороос удирдаарай.</p></div><span className="engine-badge"><Zap size={14}/> Олон холболттой таталт</span></div>
