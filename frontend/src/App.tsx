...
import { bytes, isActive, type Job, type Settings } from './types';
...
export default function App() {
  ...
  useEffect(() => {
    if (!desktop) return;
    let live = true;
    const unsubscribe = subscribe(job => { if (removed.current.has(job.id)) return; if (settingsRef.current?.notifications && (job.status==='complete' || job.status==='failed')) setNotice(`${job.filename} — ${job.status==='complete'?'Таталт дууслаа':'Таталт амжилтгүй'}`); const previous = pending.current.get(job.id); if (!previous || previous.revision < job.revision) pending.current.set(job.id,job); });
    ...
    api().GetState().then(state => { if (!live) return; setFolder(state.folder); setSettings(state.settings);settingsRef.current=state.settings; if(state.historyError) setError(state.historyError); setJobs(previous=>state.jobs.reduce(merge,previous).sort((a,b)=>b.createdAt.localeCompare(a.createdAt))); }).catch(err=>{ if(live) setError(String(err)); });
  ...
  const onRemove = useCallback((id: string) => { api().RemoveHistory(id).then(()=>{ removed.current.add(id);pending.current.delete(id);setJobs(previous=>previous.filter(j=>j.id!==id)); }).catch(err=>setError(String(err))); }, []);
  const onRetry = useCallback((id: string) => { api().RetryDownload(id).then(onCreated).catch(err=>setError(String(err))); }, [onCreated]);
  const onFile = useCallback((id: string) => { api().OpenFile(id).catch(err=>setError(String(err))); }, []);
  ...
  const active = jobs.filter(j=>['probing','downloading','canceling'].includes(j.status));
  const completed = jobs.filter(j=>j.status==='complete');
  const totalSpeed = active.reduce((sum,j)=>sum+j.progress.bytesPerSecond,0);
  const visible = jobs.filter(j=>(category==='all'||j.category===category) && (filter==='all'||filter==='queued'&&j.status==='queued'||filter==='active'&&isActive(j.status)||filter==='complete'&&j.status==='complete'||filter==='failed'&&(j.status==='failed'||j.status==='canceled')) && j.filename.toLowerCase().includes(query.toLowerCase()));
  ...
  return <MotionConfig reducedMotion="user"><div className="app-shell">
    ...
    {notice ? <div className="completion-notice" role="status">{notice}<button onClick={()=>setNotice('')} aria-label="Мэдэгдэл хаах"><X size={16}/></button></div> : null}
    {settings ? <DownloadSettings settings={settings} onSaved={s=>{setSettings(s);settingsRef.current=s;setNotice('Тохиргоо хадгалагдлаа');}} onError={setError}/> : null}
