...
export function AddDownload({ folder, setFolder, onCreated }: { folder: string; setFolder: (folder: string) => void; onCreated: (job: Job) => void }) {
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      if(!disk || !disk.enough){const data=await api().CheckDiskSpace({url,filename,folder,workers});setReport({key:requestKey,data});if(!data.enough)setError('Дискний зай хүрэлцэхгүй. Өөр хавтас сонгох эсвэл зай гаргаад дахин шалгана уу.');return;}
      const job = await api().StartDownload({ url, filename: disk.filename, folder, workers }); onCreated(job); setURL(''); setFilename(''); autoFilled.current = ''; setClipboardNotice(''); }
    catch (err) { setError(String(err)); }
  ...
  return <section className="add-panel" aria-labelledby="new-download-title">
    ...
    <form onSubmit={submit}>
      ...
      {disk ? <div className={`disk-report ${disk.enough?'':'low'}`} role="status"><strong>{disk.filename}</strong><span>Файл: {bytes(disk.total)} · Сул зай: {bytes(disk.free)} · Шаардлага: {bytes(disk.required)}</span><small>{disk.estimated ? 'Видео нэгтгэх түр зайг тооцсон ойролцоо хэмжээ.' : disk.total<0 ? 'Сервер хэмжээг өгөөгүй. Таталтын явцад сул зайг хянана.' : '67.1 MB нөөц зай багтсан. Эхлэх үед дахин шалгана.'}</small><button type="button" disabled={busy} onClick={()=>setReport(null)}>Дахин шалгах</button></div> : null}
