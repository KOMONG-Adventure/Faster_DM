...
export function DownloadSettings({ settings, onSaved, onError }: { settings: Settings; onSaved: (s: Settings) => void; onError: (message: string) => void }) {
  ...
  const [rate, setRate] = useState(String(settings.rateLimit / 1000000));
  const [notifications, setNotifications] = useState(settings.notifications);
  const [busy, setBusy] = useState(false);
  async function save() {
    const value = Number(rate);
    if (!Number.isFinite(value) || value < 0 || value > 1024) { onError('Хурд 0–1024 MB/s байна.'); return; }
    const next = { concurrent, rateLimit: Math.round(value * 1000000), notifications };
    setBusy(true);
    try { await api().SetDownloadSettings(next); onSaved(next); } catch (err) { onError(String(err)); } finally { setBusy(false); }
  }
  return <details className="download-settings"><summary>Таталтын тохиргоо · Зэрэг {settings.concurrent} · {settings.rateLimit ? `${settings.rateLimit / 1000000} MB/s / файл` : 'Хязгааргүй'}</summary>
    <div className="settings-fields">
      ...
      <label>Нэг файлын хурд (MB/s)<input type="number" min="0" max="1024" step="any" value={rate} onChange={e => setRate(e.target.value)}/></label>
